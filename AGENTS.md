...
- **explain-for-beginners**：约束/适用范围：用户自述“小白”或明确要求通俗解释时必须生效；示例：用“门牌号/钥匙”解释 gatewayUrl/token，给 3-5 步操作清单；反例：直接抛出配置项名与缩写、不解释术语；执行方式：优先使用短句、避免缩写，必要术语先用一句话解释；维护责任人：当前助手。
- **closed-loop-default-product-change**：默认每次工作视为完整的产品变更，必须闭环（验证 -> 部署/发布 -> 线上冒烟）；除非用户明确声明“仅讨论/不发布”，否则按完整流程执行，确保用户体验不中断、不被破坏。执行方式：按 AGENTS.md 的发布闭环要求与 smoke 规则执行；维护责任人：当前助手。
- **deploy-scope-by-change**：仅对发生变更的组件执行发布/部署；未改动后端或数据库则不做迁移与后端部署，未改动前端则不发布前端。用户明确要求“全量/全部发布”或组件范围时遵从用户指令。执行方式：发布闭环范围与改动范围一致；维护责任人：当前助手。
