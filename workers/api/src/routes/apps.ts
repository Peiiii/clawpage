...
appsRouter.get('/', async (c) => {
  ...
  const apps = await c.env.DB.prepare(query).bind(...params).all<App>();
...
appsRouter.get('/:id', async (c) => {
  ...
  return c.json<ApiResponse<App>>({ success: true, data: app });
});
...
appsRouter.post('/', agentAuthMiddleware, async (c) => {
  ...
  const app = await c.env.DB.prepare('SELECT * FROM apps WHERE id = ?').bind(id).first<App>();
  
  return c.json<ApiResponse<App>>({ success: true, data: app! }, 201);
});
