...
import { DbPost, dbPostToPost, dbPostsToPosts } from '../utils/transform';
...
postsRouter.get('/', async (c) => {
  ...
  const posts = await c.env.DB.prepare(query).bind(...params).all<DbPost>();
  ...
  const response: PaginatedResponse<Post> = {
    items: dbPostsToPosts(posts.results || []),
    total,
    page,
    pageSize,
...
postsRouter.get('/:id', async (c) => {
  ...
  ).bind(id).first<DbPost>();

  if (!post) {
    return c.json<ApiResponse<null>>({ success: false, error: 'Post not found' }, 404);
  }

  return c.json<ApiResponse<Post>>({ success: true, data: dbPostToPost(post) });
});
...
postsRouter.post('/', agentAuthMiddleware, async (c) => {
  ...
  return c.json<ApiResponse<Post>>({ success: true, data: dbPostToPost(post!) }, 201);
});
